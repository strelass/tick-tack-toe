...
function activate_chat(thread_id, user_name, number_of_messages) {
    ...
    function start_chat_ws() {
        ws = new WebSocket("ws://127.0.0.1:8888/chat/" + thread_id + "/");
        ws.onmessage = function(event) {
            ...
            $("div.chat div.conversation").append(
                '<div class="message">' +
                '<p class="author ' + ((message_data.sender == user_name) ? 'we' : 'partner') + '">' +
                '<span class="datetime">' + time[0] + ':' + time[1] + ':' + time[2] + '</span> ' +
                message_data.sender +
                ':</p>' +
                '<p class="message">' +
            ...
            $("div.chat p.messages").html(
                '<span class="total">' + number_of_messages["total"] + '</span> ' +
                getNumEnding(number_of_messages["total"], ["сообщение", "сообщения", "сообщений"]) +
                ' (<span class="received">' + number_of_messages["received"] + '</span> получено,' +
                ' <span class="sent">' + number_of_messages["sent"] + '</span> отправлено)');
        }
        ws.onclose = function(){
    ...
    if ("WebSocket" in window) {
        ...
        $("form.message_form").html(
            '<div class="outdated_browser_message">' +
            '<p>' +
            '   <em>Ой!</em> ' +
